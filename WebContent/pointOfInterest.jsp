...
<%@ page import="java.io.*" %>
<%@ page import="com.maphiber.MapImpl.PointOfInterest" %>
...
<th>Images</th>
  ...
  <c:forEach items="${name}" var="name">
    <tr>
      <td><b><a href="${name.value.mapUrl}"><c:out value="${name.key}" /></a></b></td>
      <td><c:out value="${name.value.formatted_address}" /><br/>
      Latitude:  &nbsp;&nbsp;&nbsp;&nbsp;<c:out value="${name.value.lat}"/><br/>
      Longitude: <c:out value="${name.value.lng}"/><br/>
      </td>
      <td><c:out value="${name.value.rating}"></c:out></td>
      <c:set var="val" value="name.value.phone_number"/>
      <c:choose>
      <c:when test="${val != 'Not available'}" ><td><a href="tel:${name.value.phone_number}"><c:out value="${name.value.phone_number}" /></a></td>
      </c:when>
      </c:choose>
      <td><img src="${name.value.imagess}"></td>
    </tr>
  </c:forEach>
</table>
</div>
